...
public class ClosePositionTrailingStop extends TradeMechanism implements MarketChangeListener{
	...
	private int I_STATE=ClosePositionTrailingStop.I_PLACING;
	...
	private int relativeStopLossTicks=1;
	...
	public ClosePositionTrailingStop(ClosePositionTrailingStopOptions cpo)
	{
		this(cpo.getDefaultListener(),
				cpo.getBetCloseInfo(),
				cpo.getMovingStopLossTicks(),
				...
				cpo.getReference(),
				cpo.getMovingAverageSamples(),
				cpo.getInitialRelativeStopLossTicks());
	}
	
	public ClosePositionTrailingStop(TradeMechanismListener botA,BetData betCloseInfoA,int stopLossTicksA, int waitFramesNormalA, int waitFramesUntilForceCloseA, int updateIntervalA, boolean forceCloseOnStopLossA,boolean useStopProfifInBestPriceA,boolean goOnfrontInBestPriceA, int startDelayA,int ignoreStopLossDelayA,int waitFramesLay1000A,int referenceA,int movingAverageSamplesA, int relativeStopLossTicksA)
	{
		super();
		...
		relativeStopLossTicks=relativeStopLossTicksA;
		
		//System.out.println("Force : "+forceCloseOnStopLoss);
		
		if(betCloseInfoA.getType()==BetData.BACK)
		{
			oddStopLoss=Utils.indexToOdd(Utils.oddToIndex(betCloseInfoA.getOddRequested())-relativeStopLossTicks);
			//oddStopLoss=Utils.indexToOdd(Utils.oddToIndex(getOddReference())-stopLossTicks);
			if(oddStopLoss==-1)
				oddStopLoss=1.01;
		}
		else
	...
	private double getOddReference()
	{
		...
		double aux=0;
		if(movingAverageSamples>1 && Utils.isValidWindow(betCloseInfo.getRd(), movingAverageSamples, 0))
		{
			if(reference==TrailingStopOptions.REF_BEST_PRICE)
			{
				...
				{
					if(betCloseInfo.getType()==BetData.BACK)
						aux+=Utils.getOddBackFrame(betCloseInfo.getRd(), i);
					else
						aux+=Utils.getOddLayFrame(betCloseInfo.getRd(), i);
				}
			...
			} else // REF_MIDLE
			{
				...
				for(int i=0;i<movingAverageSamples;i++)
				{
					aux+=((Utils.getOddLayFrame(betCloseInfo.getRd(), i)+Utils.getOddBackFrame(betCloseInfo.getRd(), i))/2.);
				}
				aux/=movingAverageSamples;
			}
		}
		else
		{
			...
			{
				if(betCloseInfo.getType()==BetData.BACK)
					aux=Utils.getOddBackFrame(betCloseInfo.getRd(), 0);
				else
					aux=Utils.getOddLayFrame(betCloseInfo.getRd(), 0);
			...
			{
				aux=((Utils.getOddLayFrame(betCloseInfo.getRd(), 0)+Utils.getOddBackFrame(betCloseInfo.getRd(), 0))/2.);
			}
		}
	...
	private void updateOddStopLoss()
	{
		if(betCloseInfo.getType()==BetData.BACK)
		{
			...
			ref=Utils.indexToOdd(Utils.oddToIndex(ref)-stopLossTicks);
	...
	private void refresh()
	{
		...
		System.out.println("Stop Loss : "+oddStopLoss);
		if (pause) return;
	...
	public void setPause(boolean pauseA) {
		pause=pauseA;
		writeMsgToListeners("ClosePositionTralingStop setPause() was called with :"+pause, Color.BLUE);
	}
	...
	private void writeMsgToListeners(String msg, Color color)
	{
		if(listeners!=null)
		{
			String msgaux="[ClosePositionTralingStop]";
