...
public class Swing{ // implements TradeMecanism{
	...
	public int STATE= Swing.UNDEFINED;
	...
	private int countFramesWaitOpen=0;
	...
	public Swing(MarketData Market, RunnersData rdA, double stakeSize, double entryOddA,int waitFramesOpenA, int waitFramesNormalA,int waitFramesEmergencyA, Bot botOwner,int directionA,int ticksUpA,int ticksDownA)
	{
		marketData=Market;
		rd=rdA;
	...
	public void initialize()
	{
		...
		bot.setInTrade(true);
		if(marketData.isInPlay() && Parameters.jump_to_the_next_race)
		{
	...
	public void updateState()
	{
		...
		else
		{
			if(processing==false)
			{
				processing=true;
				switch (getSTATE()) {
		            case Swing.UNDEFINED:  clean();       break;
		            case Swing.START:  openTrade();      break;
		            case Swing.WAIT_OPEN:  waitOpenTrade();      break;
		            case Swing.WAIT_CLOSE:  waitCloseTrade();      break;
		            case Swing.CLOSE: closeTrade();  break;
		            case Swing.CLOSE_EMERGENCY:  closeEmergencyTrade(); break;
		            case Swing.EDGE: processEdge();  break;
		            case Swing.EXIT: exit();      break;
		            default: clean(); break;
				}
				processing=false;
			}
...
//		if(direction<0)
//			actualOddEntry = Utils.indexToOdd(Utils.oddToIndex(getRunnerOddBak())+1);
//		else
//			actualOddEntry =  Utils.indexToOdd(Utils.oddToIndex(getRunnerOddLay())-1);
	...
	{
		writeMessageText("#### WAIT OPEN #### Frame : "+countFramesWaitOpen, Color.BLACK);
		countFramesWaitOpen++;
		if(direction<0)
		{
			// predict Odd down
			...
			else
			{
				...
				if(entryOdd>getRunnerOddLay() || countFramesWaitOpen>waitFramesOpen)
				{
					if(cancelBetID(betIdBack)==-1)
					{
		...
		else
		{
			...
			{
				if(entryOdd<getRunnerOddBak() || countFramesWaitOpen>waitFramesOpen)
				{
					if(cancelBetID(betIdLay)==-1)
					{
	...
	private void closeTrade() {
		...
		if(direction<0)
		{
			...
			if (countFramesWaitEmergency > waitFramesEmergency) {
				...
				setSTATE(Swing.CLOSE_EMERGENCY);
				return;
			}
	...
	public void forceClose() {
		countFramesWaitOpen=waitFramesOpen;
