...
public class ProcessInputRawValData {
	...
	public static Vector<double[]> loadFileIntoMemory(String fileName)
	{
		...
		try {
			...
			{
				String saux[]=s.split(" ");
				double dataExample[]=new double[DataWindowsSizes2018.TIME_SERIES]; 
	...
	public static JFrame showHistogramChart(Histogram histogram,Vector<Double> points,String name)
	{
		...
		for(double point:points)
		{
			...
			chart.addValue("interval", point, 1000, Color.RED);
			chart.addValue("interval", point+1, 0, Color.RED);
		}
	...
	{
		double minmax[][]=new double[DataWindowsSizes2018.TIME_SERIES][2];
		...
		for(int i=0;i<DataWindowsSizes2018.TIME_SERIES;i++)
		{
			...
			minmax[i][0]=histogram.getMinFiltred(filters[i]);
			minmax[i][1]=histogram.getMaxFiltred(filters[i]);
...
//			}
	...
	public static Vector<double[]> normalize(Vector<double[]> examples,double[][] minmax)
	{
		...
		{
			double normalExample[]=new double[DataWindowsSizes2018.TIME_SERIES];
	...
	public static Vector<Double[]> normalizeDouble(Vector<Double[]> examples,double[][] minmax)
	{
		Vector<Double[]> ret=new Vector<Double[]>();
	...
	public static double[][] loadMinMaxFileIntoMemory(String file,Vector<CategoryNode> cat)
	{
		...
		String s;
		try {
			for(int line=0;line<DataWindowsSizes2018.TIME_SERIES;line++)
			{
	...
	public static void main(String[] args) {
			...
			double filters[] =new double[DataWindowsSizes2018.TIME_SERIES];
			...
			//for(int i=0;i<648;i++)
			{
				...
				String fileName=CategoryNode.getAncestorsStringPath(cat)+"NNRawDataIn2018Val.csv";
				...
				if(file.exists()) { 
					...
					double minmax[][]=loadMinMaxFileIntoMemory("NNMinMax.csv",cat);
					...
					// para deixar no original
					writeTalbleFile(normalizeExamples,cat,"NNNormalizeDataInVal.csv");
