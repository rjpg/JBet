...
public class ExtractTradingParameters {
...
//				}
		...
		public static double[][] findParameters(Vector<double[]> examples,double[][] minmax)
		{
			
			double[][] ret =new double[4][2];
			
			
			Vector<Double[]> retSD=new Vector<Double[]>();
			Vector<Double[]> retWD=new Vector<Double[]>();
			Vector<Double[]> retSU=new Vector<Double[]>();
			Vector<Double[]> retWU=new Vector<Double[]>();
			...
			for(double[] rawExample:examples)
			{
				...
				if(rawExample[0]<minmax[0][0])
				{
					...
					values[0]=rawExample[3];
					values[1]=rawExample[2];
					retSD.add(values);
				...
				else if (rawExample[0]>minmax[1][1] && rawExample[0]<minmax[0][1])
				{
					//normalExample[0]=3;
					Double[] values=new Double[2];
					values[0]=rawExample[2];
					values[1]=rawExample[3];
					retWU.add(values);
				}
			...
			ret[0][0]=auxAvgmax/retSD.size();
			ret[0][1]=auxAvgmin/retSD.size();
			
			auxAvgmax=0;
			auxAvgmin=0;
			for (Double[] val:retWD)
			{
				auxAvgmax+=val[0];
				auxAvgmin+=val[1];
			}
			
			ret[1][0]=auxAvgmax/retWD.size();
			ret[1][1]=auxAvgmin/retWD.size();
			
			auxAvgmax=0;
			auxAvgmin=0;
			for (Double[] val:retWU)
			{
				auxAvgmax+=val[0];
				auxAvgmin+=val[1];
			}
			
			ret[2][0]=auxAvgmax/retWU.size();
			ret[2][1]=auxAvgmin/retWU.size();
			
			auxAvgmax=0;
			auxAvgmin=0;
			for (Double[] val:retSU)
			{
				auxAvgmax+=val[0];
				auxAvgmin+=val[1];
			}
			
			ret[3][0]=auxAvgmax/retSU.size();
			ret[3][1]=auxAvgmin/retSU.size();
		...
		public static void writeTalbleFile(double[][] examples,Vector<CategoryNode> cat,String fileNameA)
		{
		...
		public static void main(String[] args) {
				...
				//for(int i=0;i<648;i++)
				{
					...
					if(file.exists()) { 
						...
						double[][] normalizeExamples=findParameters(collectExamples,minmax);
						for(double[] x:normalizeExamples)
						{
							System.out.println("------------");
						...
						// para deixar no original
						writeTalbleFile(normalizeExamples,cat,"TradingParams.csv");
