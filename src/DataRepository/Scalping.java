...
public class Scalping //implements TradeMecanism//implements MarketChangeListener 
{
	...
	public int STATE= Scalping.UNDEFINED;
	...
	public Scalping(MarketData Market, RunnersData rdA, double stakeSize, double entryOddA,int waitFramesNormalA,int waitFramesEmergencyA, Bot botOwner,int directionA)
	{
		marketData=Market;
		rd=rdA;
	...
	public void initialize()
	{
		...
		{
			writeMessageText("Did not enter Scalping Becouse Market is inplay: exit", Color.RED);
			exit();
		}
		//16899726555
		...
		setSTATE(Scalping.START);
		if(!Parameters.simulation)
		{
			as = new ScalpingThread();
			t = new Thread(as);
			t.start();
		}
	...
	public void updateState()
	{
			...
			if (rd != null) {
				...
				if (odd > entryOdd)
					if (direction > 0) {
						...
						bot.tradeResults(rd, 1, direction, entryOdd, odd, stake,0,0,Math.abs(Utils.oddToIndex(odd)-Utils.oddToIndex(entryOdd)));
						this.clean();
					} else {
		...
		else
		{
			if(processing==false)
			{
				processing=true;
				switch (getSTATE()) {
		            case Scalping.UNDEFINED:  clean();       break;
		            case Scalping.START:  openTrade();      break;
		            case Scalping.WAIT_OPEN:  waitOpenTrade();      break;
		            case Scalping.WAIT_CLOSE:  waitCloseTrade();      break;
		            case Scalping.CLOSE: closeTrade();  break;
		            case Scalping.CLOSE_EMERGENCY:  closeEmergencyTrade(); break;
		            case Scalping.EDGE: processEdge();  break;
		            case Scalping.EXIT: exit();      break;
		            default: clean(); break;
				}
				processing=false;
			}
	...
	private void waitOpenTrade()
	{
		...
		if(direction<0)
		{
			...
			if(amountBack!=0 )
			{
				...
				oddToCloseLay=Utils.indexToOdd(Utils.oddToIndex(entryOdd)-1);
		...
		else
		{
			...
			if(amountLay!=0)
			{
				...
				oddToCloseBack=Utils.indexToOdd(Utils.oddToIndex(entryOdd)+1);
	...
	private void waitCloseTrade() {
		...
		if(direction<0)
		{	
			if(entryOdd<getRunnerOddBak()) // Go after
			{
				writeMessageText("waitCloseTrade(): entryOdd<getRunnerOddBak() ("+entryOdd+"<"+getRunnerOddBak()+") : CLOSE", Color.RED);
				setSTATE(Scalping.CLOSE);
				return;
			}
		...
		{
			if(entryOdd>getRunnerOddLay()) // Go after
			{
				writeMessageText("waitCloseTrade(): entryOdd>getRunnerOddLay() ("+entryOdd+"<"+getRunnerOddLay()+") : CLOSE", Color.RED);
				setSTATE(Scalping.CLOSE);
				return;
			}
	...
	private void closeTrade() {
		...
		if(direction<0)
		{
			...
			if (countFramesWaitEmergency > waitFramesEmergency) {
				...
				setSTATE(Scalping.CLOSE_EMERGENCY);
				return;
			}
	...
	public void exit()
	{
		...
		writeMessageText("Exit Scalping ", Color.BLUE);
		clean();
	}
	...
	public void clean()
	{
	 ...
	 private ScalpingThread as;
	 private volatile Thread t;
	 
	 public class ScalpingThread extends Object implements Runnable {
